...
// to load particular device from the "TeensyDAC.dll" library
...
int CDemoCamera::Initialize()
{
   ...
   TeensyHub* pHub = static_cast<TeensyHub*>(GetParentHub());
   if (pHub)
   {
   ...
   // CameraName
   nRet = CreateStringProperty(MM::g_Keyword_CameraName, "TeensyDAC-MultiMode", true);
...
/**
 * Callback function that will be called by TeensyDAC everytime
...
void TeensyHub::GetName(char* pName) const
{
